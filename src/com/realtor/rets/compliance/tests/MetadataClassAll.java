 ...
 * @author pobrien
 */
public class MetadataClassAll extends BaseEvaluator {

  public MetadataClassAll() {
    super();
    specReference = "";
  }
...
private String errorList="";
  ...
  protected TestResult processResults(String transName, RETSTransaction t) {
      ...
      int cnt=CollectionUtils.keyCount(m,"content-type");
        ...
        if (cnt > 1)
        {
            ...
            return reportResult("MetadataClass*", mContent,"Info",note,"n/a");
   ...
   */
  protected TestResult validateClassAll(String transactionName,
                                       String responseBody) {
    ...
    String jException="";
	  ...
	  boolean found=(responseBody.indexOf("<METADATA-TABLE")>=0)
				||(responseBody.indexOf("<METADATA-UPDATE")>=0)
				||(responseBody.indexOf("<METADATA-COLUMN_GROUP_SET")>=0)
				||(responseBody.indexOf("<METADATA-COLUMN_GROUP")>=0);


		if(found){
		return reportResult("MetadataClass*:  "+transactionName,
		                        "Checks to see if the transaction has valid breadth and depth",
		                        "Success", notes,jException,"n/a");
  		}else{
		 return reportResult("MetadataClass*:  "+transactionName,
                        "Checks to see if the transaction has valid breadth and depth",
                        status, "No child metadata was found beneath METADATA-CLASS: "+notes,jException,"n/a");
  		}
	}
		 return reportResult("MetadataClass*:  "+transactionName,
			                           "Checks to see if the transaction has valid breadth and depth",

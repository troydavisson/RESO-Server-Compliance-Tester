 ...
 * @author pobrien 
 */
public class DMQLSystemNumericResultsMin extends DMQLResultsSystem {
	
    private static Log log = LogFactory.getLog(DMQLSystemNumericResultsMin.class);
    ...
    protected void compareDMQLResults(String transName, String responseBody, 
        ...
        if (log.isDebugEnabled()){
            log.debug("* In DMQLSystemNumericResultsMin evaluator class");            
        }
        
        String fieldName        = getDMQLProperty(DMQL_DECIMAL_MINIMUM_FIELD);
        String fieldValue       = getDMQLProperty(DMQL_DECIMAL_MINIMUM_VALUE);
        ...
        if (dataList.isEmpty()) {
            ...
            log.error("DMQLSystemNumericResultsMax Failed - " + testResultNotes);
        }
